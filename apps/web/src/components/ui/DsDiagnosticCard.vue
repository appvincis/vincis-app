  ...
  <div class="bg-surface-container-low rounded-xl p-8 relative overflow-hidden soft-brutalist-border group">
    ...
    <div class="relative z-10 flex flex-col h-full">
      ...
      <div v-if="isLoading" class="flex flex-col items-center justify-center py-8 space-y-4">
        <VSpinner />
        <p class="text-sm font-label text-on-surface-muted animate-pulse">A Inteligência Artificial está analisando seu desempenho...</p>
      </div>

      <!-- Estado Upsell (Basic Plan) -->
      <div v-else-if="!isPremium" class="space-y-4 max-w-2xl text-left">
        <div class="p-4 bg-surface-container-highest border border-outline-variant/30 rounded-xl relative overflow-hidden">
          ...
          <div class="relative z-10 flex flex-col gap-3">
            ...
            <p class="text-sm font-body text-on-surface-variant leading-relaxed">
              Desbloqueie o Diagnóstico IA para receber análises profundas sobre sua curva de retenção, pontos de fadiga cognitiva e dicas personalizadas para o seu estudo.
            </p>
            <div class="mt-2">
              <router-link to="/private/premium" class="inline-flex items-center gap-2 px-4 py-2 bg-amber-500 text-amber-950 rounded-lg text-xs font-bold hover:bg-amber-400 transition-all active:scale-95 shadow-sm">
      ...
      <div v-else-if="!hasData" class="space-y-4 max-w-2xl text-left">
        ...
        <p class="text-sm font-body text-secondary leading-relaxed">
          Nós utilizamos seus últimos 30 dias de sessões de estudo e registros de erro para montar um relatório detalhado e identificar onde você pode focar sua revisão.
        </p>
        <div class="mt-4">
          <button @click="$emit('generate')" class="px-4 py-2.5 bg-primary text-on-primary rounded-lg text-xs font-bold hover:bg-primary/90 transition-all flex items-center gap-2 active:scale-95 shadow-sm">
      ...
      <div v-else class="space-y-4 max-w-2xl text-left animate-fade-in">
        <p class="text-lg font-headline italic text-on-surface-variant leading-relaxed">
          "Seu desempenho em <span class="text-on-surface font-bold">{{ highlightDiscipline }}</span> mostra uma retenção <span class="text-on-surface font-bold">{{ retentionRate }}</span>. No entanto, observamos uma fadiga cognitiva ou repetição de erros em <span class="text-on-surface font-bold">{{ fatigueDiscipline }}</span>."
        </p>
        <p class="text-sm font-body text-secondary leading-relaxed">
